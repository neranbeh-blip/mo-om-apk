...
class MainActivity:Activity(){
 ...
 private fun settings(){page("Settings","Agent security and integration"){c->c.addView(info("Agent account","MoPay Agent 01 • +237000000002",gray));c.addView(channel("Security","Operator authorization and audit controls","⌁"){});c.addView(channel("Integration","MTN/Orange API connection status","↔"){ex.execute{try{val r=SupabaseClient.rpc("operator_integration_status");runOnUiThread{AlertDialog.Builder(this).setTitle("Integration status").setMessage(r.toString(2)).setPositiveButton("OK",null).show()}}catch(e:Exception){runOnUiThread{fail(e.message?:"Connection error")}}}})}}
